...
left = cv2.imread('../data/walnut_left.jpg')
right = cv2.imread('../data/walnut_right.jpg')

#taken from same camera, no initial cropping or change of setting
left = cv2.resize(left, ((left.shape[1])//3, (left.shape[0])//3))
right = cv2.resize(right, ((right.shape[1])//3, (right.shape[0])//3)) 
'''
This is via the SIFT method for feature point extraction and CV2 built in homography func
'''
...
cv2.imwrite('../data/Panaroma_with_SIFT.jpg', panaroma)
cv2.imwrite('../data/Panaroma_with_BRIEFandFAST.jpg', panaroma2)